...
Object.freeze(locationLookupLimiter);
...
export async function registration(
  ...
  try {
    let location = await locationLookupLimiter.schedule(() =>
      utils.addressToCoordinates(body.address)
    );
  ...
  try {
    ...
    if (!doc) throw new Error('Saving document failed');
