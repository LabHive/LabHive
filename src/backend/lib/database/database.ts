...
export async function connect(): Promise<any> {
  return mongoose.connect(`mongodb://${userPassword}${connectionBase}:27017/labshare?authSource=admin`, { useNewUrlParser: true }).then(async () => {
    if (CONF.ADMIN_USERS) {
...
export function cleanUserObjForToken(token: Optional<JWT>, user: IUserCommon) {
  delete user._id
  delete user.__v
