...
import { getModelForRole, getUser, getFilterForPublicUsers, cleanUserObjForToken, sensibleUserProjection } from '../../lib/database/database';
import { UserCommon } from "../../lib/database/models";
...
function buildFilter(req: express.Request, res: express.Response, token?: JWT): Optional<any> {
...
function validSearchFilter(req: express.Request, res: express.Response, token?: JWT): boolean {
...
async function getZipcodeCoords(req: express.Request, res: express.Response, token?: JWT): Promise<Optional<number[]>> {
    if (req.query.zipcode) {
